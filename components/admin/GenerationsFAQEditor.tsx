...
interface RowState extends GenerationsFaqItem {
  /** Solo para la key de React en esta sesión de edición — nunca se envía al servidor. */
  key: string;
}

let keySeq = 0;
const nextKey = () => `faq-${Date.now()}-${keySeq++}`;

/** Fork de PracticalInfoEditor con los campos renombrados pregunta/respuesta. */
export function GenerationsFAQEditor({ defaultItems }: { defaultItems: GenerationsFaqItem[] }) {
  const [rows, setRows] = useState<RowState[]>(() => defaultItems.map((item) => ({ ...item, key: nextKey() })));

  function updateRow(key: string, patch: Partial<GenerationsFaqItem>) {
    setRows((prev) => prev.map((r) => (r.key === key ? { ...r, ...patch } : r)));
  }

  function addRow() {
    setRows((prev) => [...prev, { key: nextKey(), q: "", a: "" }]);
  }
      ...
      {rows.map((row, i) => (
        <div key={row.key} className="space-y-3 rounded-lg border border-border p-4">
          <div className="flex items-start justify-between gap-3">
            <div className="flex-1">
              <TextField
                ...
                name={`faq.${i}.q`}
                defaultValue={row.q}
                onChange={(e) => updateRow(row.key, { q: e.target.value })}
                required
              />
            </div>
          ...
          <TextAreaField
            ...
            name={`faq.${i}.a`}
            defaultValue={row.a}
            onChange={(e) => updateRow(row.key, { a: e.target.value })}
            rows={3}
            required
          />
        </div>
